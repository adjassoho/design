...
export const OrganizerDashboard: React.FC<OrganizerDashboardProps> = ({
  ...
  return (
    <div className="w-full flex-1 overflow-y-auto p-4 space-y-4 font-sans text-neutral-200">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-neutral-900/90 border border-amber-500/30 rounded-3xl p-4 shadow-xl">
        <div className="flex items-center gap-3">
          <img
            src={memorial.portraitUrl}
            alt={memorial.fullName}
            className="w-12 h-12 rounded-2xl object-cover border border-amber-400/80 shadow"
          ...
          <div>
            ...
            <p className="text-[11px] text-neutral-400">
              {memorial.serviceOfSongs.venueName} • Culte d'Obsèques
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          {!memorial.isPaid && (
            <button
              ...
              className="px-3.5 py-2 bg-gradient-to-r from-amber-600 via-amber-500 to-amber-600 hover:from-amber-500 text-neutral-950 font-bold rounded-xl text-xs flex items-center gap-1.5 shadow cursor-pointer"
            >
              <Zap className="w-3.5 h-3.5 fill-neutral-950" />
              <span>Publier avec FedaPay (500 F)</span>
            </button>
          )}
          <button
            onClick={onEditProgram}
            className="px-3 py-2 bg-neutral-800 hover:bg-neutral-700 rounded-xl text-neutral-200 text-xs font-semibold"
          >
            Modifier Faire-part
          </button>
        </div>
      ...
      <div className="flex border-b border-neutral-800 pb-1 gap-4 text-xs font-semibold">
        ...
        >
          Gestionnaire d'Invités ({personalGuests.length})
        </button>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
            <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-3 flex flex-col justify-between">
              <div className="flex items-center justify-between text-neutral-400">
                <span className="text-[10px] uppercase font-bold tracking-wider">Liens Envoyés</span>
                <Users className="w-4 h-4 text-amber-400" />
              </div>
              <div className="mt-2">
                <span className="text-2xl font-bold font-mono text-white">{personalGuests.length}</span>
                <span className="text-[10px] text-neutral-500 block">/ 150 inclus</span>
              </div>
            </div>

            <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-3 flex flex-col justify-between">
              <div className="flex items-center justify-between text-neutral-400">
                <span className="text-[10px] uppercase font-bold tracking-wider">Ouvertures</span>
              ...
              <div className="mt-2">
                ...
                <span className="text-[10px] text-neutral-500 block">
                  {personalGuests.length > 0
                    ? `${Math.round((totalOpened / personalGuests.length) * 100)}% de taux`
                    : 'En attente'}
                </span>
              </div>
            </div>
            ...
            <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-3 flex flex-col justify-between">
              <div className="flex items-center justify-between text-neutral-400">
                <span className="text-[10px] uppercase font-bold tracking-wider">Présences RSVP</span>
            ...
            <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-3 flex flex-col justify-between">
              <div className="flex items-center justify-between text-neutral-400">
                <span className="text-[10px] uppercase font-bold tracking-wider">Livre d’Hommages</span>
                <Flame className="w-4 h-4 text-amber-400" />
              </div>
              <div className="mt-2">
                <span className="text-2xl font-bold font-mono text-amber-300">{tributesCount}</span>
                <span className="text-[10px] text-neutral-500 block">Témoignages & Bougies</span>
              </div>
            </div>
          </div>

          {/* Broadcast / Mode B Section */}
          <div className="bg-neutral-900/80 border border-amber-500/30 rounded-2xl p-4 space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Share2 className="w-4 h-4 text-amber-400" />
                <h3 className="font-cinzel text-xs font-bold text-amber-200 uppercase tracking-wider">
                  Mode B — Lien Public & Statut WhatsApp
                </h3>
              </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="px-3 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 cursor-pointer"
              >
          ...
          <div className="bg-neutral-950 border border-neutral-800 rounded-2xl p-3 flex items-center justify-between">
            <div className="space-y-0.5">
              <p className="text-xs font-semibold text-white">Tester l’expérience invité</p>
              <p className="text-[11px] text-neutral-400">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="px-3 py-1.5 bg-neutral-800 hover:bg-neutral-700 text-amber-200 rounded-xl text-xs font-semibold flex items-center gap-1"
              >
                <span>Vue Nominative</span>
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            ...
            <button
              ...
              className="px-3.5 py-2 bg-gradient-to-r from-amber-600 to-amber-500 text-neutral-950 font-bold rounded-xl text-xs flex items-center justify-center gap-1.5 cursor-pointer"
            >
          ...
          {showAddForm && (
            <form
              ...
              className="bg-neutral-900 border border-amber-500/40 rounded-2xl p-3.5 space-y-3"
            >
              <h4 className="font-cinzel text-xs font-bold text-amber-300 uppercase tracking-wider">
                Nouvel Invité Nominatif
              </h4>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                <div className="sm:col-span-2">
                  ...
                  <input
                    type="text"
                    required
                    placeholder="Ex: Tonton Emmanuel & Famille"
                    value={newGuestName}
              ...
              <div className="flex justify-end gap-2 pt-1">
                ...
                <button
                  type="submit"
                  className="px-4 py-1.5 bg-amber-500 text-neutral-950 font-bold rounded-xl text-xs"
                >
                  Enregistrer & Générer Lien
                </button>
              </div>
            </form>
          )}
          ...
          <div className="space-y-2">
            {filteredGuests.map((guest) => (
              <div
                key={guest.id}
                className="bg-neutral-900/90 border border-neutral-800 hover:border-amber-500/40 rounded-2xl p-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3 transition-all"
              >
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    {guest.rsvpStatus === 'yes' && (
                      <span className="text-[10px] px-2 py-0.5 bg-emerald-950 border border-emerald-500/40 text-emerald-300 rounded-full flex items-center gap-1 font-medium">
                        ...
                        Présent ({guest.rsvpSeats || guest.seats})
                      </span>
                    )}
                  ...
                  <div className="flex items-center gap-3 text-[11px] text-neutral-400">
                    <span className="flex items-center gap-1">
                      ...
                      {guest.openCount} ouverture{guest.openCount > 1 ? 's' : ''}
                    </span>
                    {guest.phone && (
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button
                    ...
                    className="p-2 bg-neutral-800 hover:bg-neutral-700 text-amber-300 rounded-xl text-xs"
                    title="Aperçu de ce que voit l'invité"
                  >
